...
class UserQueries(IUserQueries):
    ...
    def update_user(
        ...
        with self.session as session:
            ...
            session.query(UserModel).filter_by(user_id=old_user.user_id).update(
                data
            )
            session.commit()
            user_updated = (
                session.query(UserModel)
                .filter_by(user_id=old_user.user_id)
                .first()
            )
        return user_updated
    ...
    def get_user_byemail(self, user_email: str) -> UserModel:
        ...
        with self.session as session:
            user_obj = (
                session.query(UserModel)
                .filter_by(email=user_email, is_active=True)
                .first()
            )
        return user_obj
    ...
    def get_admin_emails(self):
        with self.session as session:
            admin_users = (
                session.query(UserModel).filter_by(rol=UserRoles.admin).all()
            )
