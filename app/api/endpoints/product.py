...
def create_product(
    ...
    current_user: ProductDBSchema = Security(get_current_user_from_service, scopes=[UserRoles.admin.value])
    ) -> ProductDBSchema:
...
def get_product_by_id(
    ...
    product_service: ProductService = Depends(get_product_services), 
    token: str = Depends(AuthSchema),
    current_user: ProductDBSchema = Security(get_current_user_from_service, scopes=[UserRoles.admin.value, UserRoles.anonymous.value])
    ) -> Optional[ProductDBSchema]:
    product = product_service.get_product_by_id(product_id)
    if product:
        try:
            if current_user.rol == UserRoles.anonymous:
                view_event = ViewProductByAnon(user_id=current_user.user_id, product_id=product.product_id, date=datetime.now())
            return product
        finally:
...
def remove_product(
    ...
    product_removed: ProductDBSchema = product_service.remove_product(product_id)
